...
<nav class="navbar navbar-expand-lg bg-body-tertiary sticky-top" id="nav1">
  ...
  <div class="container py-5 col-10">
    ...
    <fieldset class="field rounded-3 p-4">
      <form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>" method="POST">
        <div class="row">
          <div class="mb-3 col">
            <input type="text" class="form-control" maxlength="200" placeholder="Descrição Especialidade" name="Especialidades[]" required>
          </div>
  ...
  <script>
    ...
    btnIncluiProc[0].addEventListener("click", () => {
      ...
      newInput.placeholder = "Descrição Especialidade";
      newInput.name = "Especialidades[]";
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
  try {
    for ($i = 0; $i < count($Especialidades); $i++) {
      $slct = $conn->prepare("SELECT * FROM especialidades WHERE descEspecialidade = ?");
      $slct->bind_param("s", $Especialidades[$i]);
      $slct->execute();
      $slct = $slct->get_result();
      $slct = $slct->fetch_assoc();
      if ($slct != null) {
        echo "<script>alert('Especialidade" . " " . $Especialidades[$i] . " " . "Já cadastrada');</script>";
      } else {
        $in .= ("INSERT INTO especialidades (descEspecialidade) VALUES ('$Especialidades[$i]');");
      }
    }

    if ($in != null && $conn->multi_query($in) == TRUE) {
      echo "<script>alert('Especialidade(s) Cadastrada');</script>";
    }
