...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
  try {
    ...
    if ($select->num_rows == 0) {
      ...
      $select2 = "SELECT idPaciente FROM pacientes WHERE cpfPaciente = ?";
      ...
      if ($slct != null) {
        ...
        if ($conn->errno == 0) {
            $data = date("Y-m-d");
            $inst = $conn->prepare("INSERT INTO comissionados (idPaciente, idParceiro, dataCadastro, idVendedor) VALUES (?, ?, ?, ?)");
            $inst->bind_param("iisi", $slct['idPaciente'], $_SESSION['idParVend'], $data, $_SESSION['idVend']);
            $inst->execute();
            $inst->store_result();
            if ($inst->affected_rows > 0) {
              $statusCartao = 0;
              $inst2 = $conn->prepare("INSERT INTO cartoes (idPaciente, statusCartao)
                                              VALUES (?, ?)");
              $inst2->bind_param("ii", $slct['idPaciente'], $statusCartao);
              $inst2->execute();
              $inst2->store_result();
              if($inst2->affected_rows > 0){
      ...
      $_SESSION['message'] = "Paciente com CPF já cadastrado em nosso sistema.";
      $conn->rollback();
      $conn->close();
      header("Location: ./cadPaciente.php");
    }
